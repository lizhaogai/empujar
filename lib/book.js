...
book.prototype.pidFile = function(){
  var self = this;
  return self.options.pidsPath + '/' + self.options.name.replace(/\s/g, '_') + '.pid';
};

book.prototype.ensurePid = function(){
  var self = this;
  var pidFile = self.pidFile();
  self.log('PidFile => ' + pidFile, 'debug' );
  ...
  if(fs.existsSync(pidFile)){
    ...
    if( isrunning(parseInt(oldpid)) ){
      ...
      self.log('removing old pidfile');
    }
  }

  fs.writeFileSync(pidFile, Buffer.from([process.pid]));
};
        ...
        if(connection.connection){
          ...
          connection.connection.connect(function(error){
            if(error){
              ...
              if(self.options.failOnConnectionError === true){
                next(e);
              }else{
                next();
              }
